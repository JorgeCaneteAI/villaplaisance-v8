...
<?php /**
 * Vue : Votre hôte (Jorge).
 * Branchée sur les données dynamiques de l'admin :
 *   - vp_host_profile (name, subtitle, photo, intro, origin, passions, philosophy, fun_facts, quote)
 *   - vp_host_blocks  (title_{lang}, content_{lang}, image, position) — blocs CV
 *   - vp_reviews      (avis mentionnant Jorge / Georges / George)
 * Layout : front-proto.
...
// Sépare prénom / nom pour le H1 stylisé (italique sur le nom)
$nameParts = preg_split('/\s+/', trim($name), 2);
$firstName = $nameParts[0] ?? $name;
$lastName  = $nameParts[1] ?? '';

// Initiales pour le placeholder portrait (si pas de photo)
$initials = '';
